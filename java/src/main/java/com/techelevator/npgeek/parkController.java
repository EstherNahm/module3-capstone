...
public class parkController {
	...
	public String showSelectedPark(ModelMap model, @RequestParam String parkCode, @RequestParam String tempUnit,
			HttpSession session) {
		// to hash out the added , at the end of the parkCode
		parkCode = parkCode.endsWith(",") ? parkCode.substring(0, parkCode.length() - 1) : parkCode;
	...
	public String processUserSurveyInput(HttpSession session, @RequestParam String emailAddress,
			@RequestParam String state, @RequestParam String activityLevel, @RequestParam String parkCode) {
