...
<body>
	...
	<div class="selectedpark">
		...
		<div class="parkfacts">
			...
			<p id="parkfacts">
				<strong>ABOUT:</strong><br /> Size: ${park.acreage} acres<br />
				State: ${park.state}<br /> Elevation: ${park.elevationInFeet} feet<br />
				Trails: ${park.milesOfTrail} miles<br /> Campsites:
				${park.numberOfCampsites}<br /> Climate: ${park.climate}<br /> Year
				founded: ${park.yearFounded}<br /> Annual visitors:
				${park.annualVisitorCount}<br /> Entrance fee: $${park.entryFee}<br />
				Animal species: ${park.numberOfAnimalSpecies}<br />
		</div>

		<div class=parkdescription>
			<br />
			<h3 id="parkquote">${park.inspirationalQuote}
				-${park.inspirationalQuoteSource}</h3>
			<br />
			<p id="parkdescription">${park.parkDescription}<br />
				<br />
		</div>
		...
		<div class="tempbuttons">
			...
			<form method="POST" action="${formAction}">
				...
				<div class="buttons">
					<input class="button" type="submit" value="F" name="tempUnit"
						id="F"> <input class="button" type="submit" value="C"
						name="tempUnit" id="C">
				</div>
			</form>
		</div>
				...
				<div id="weatherforecast">
					<ul>
						...
						<c:if test="${tempUnit == 'F'}">
							...
							<li><strong>LOW:</strong> ${weather.low}°F<br />
							<br />
						</c:if>

						<c:if test="${tempUnit == 'C'}">
							<li><strong>HIGH:</strong> <fmt:formatNumber type="number"
									maxFractionDigits="0" value="${(weather.high - 32) * .5556}" />°C<br />
							<li><strong>LOW:</strong> <fmt:formatNumber type="number"
									maxFractionDigits="0" value="${(weather.low - 32) * .5556}" />°C<br />
							<br />
						</c:if>
					</ul>
					<c:choose>
						<c:when test="${weather.forecast == 'partly cloudy'}">
							...
							<a><img id="weatherimage" src="${weatherImage}" /></a>
							<br />
						</c:when>
					...
					<div class="bold">
						<c:choose>
							<c:when test="${weather.forecast == 'snow'}">Pack snowshoes.<br />
							</c:when>
							<c:when test="${weather.forecast == 'rain'}">Bring rain gear and waterproof shoes.<br />
							</c:when>
							<c:when test="${weather.forecast == 'thunderstorms'}">Seek shelter and avoid hiking on exposed ridges.<br />
							</c:when>
							<c:when test="${weather.forecast == 'sunny'}">Pack sunblock.<br />
							</c:when>
						</c:choose>
