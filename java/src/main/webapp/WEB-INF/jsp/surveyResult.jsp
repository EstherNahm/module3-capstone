		...
		<div class="favoritepark">
			<c:forEach var="favoritepark" items="${favoriteParks}" varStatus="s">
				...
				<div class="grid2">
					...
					<a href="${parkDetailPath}"><img src="${parkImage}"
						class="parkrank" /></a>
					...
					<div class="flex">
						<h1 class="parkrank">
							#
							<c:out value="${s.count}" />
							${favoritepark.park.parkName}
						</h1>
						<br />
						<p class="parkrank">

							<strong>TOTAL VOTES: ${favoritepark.surveyCount}</strong><br /> <br />
							<br />${favoritepark.park.parkDescription}</p>
					</div>
				</div>
			</c:forEach>
		</div>
